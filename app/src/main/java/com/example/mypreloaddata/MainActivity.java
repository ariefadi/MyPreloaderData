    ...
    */
    ...
    private static class IncomingHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case DataManagerService.PREPARATION_MESSAGE:
                    weakCallback.get().onPreparation();
                    break;
                case DataManagerService.UPDATE_MESSAGE:
                    Bundle bundle = msg.getData();
                    long progress = bundle.getLong("KEY_PROGRESS");
                    weakCallback.get().updateProgress(progress);
                    break;
                case DataManagerService.SUCCESS_MESSAGE:
                    weakCallback.get().loadSuccess();
                    break;
                case DataManagerService.FAILED_MESSAGE:
                    weakCallback.get().loadFailed();
                    break;
                case DataManagerService.CANCEL_MESSAGE:
